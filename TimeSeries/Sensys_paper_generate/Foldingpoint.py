...
def findEK(data: pd.DataFrame, sensorNum: int, parameters: EKGroupParameter):
    ...
    for i in range(
        ...
        if (
            parameters.convexDec.online == True
            and len(kneedle.all_knees) == 1
            or parameters.convexDec.online == False
            and kneedle.knee != None
        ):
...
def complement(
    ...
    for j, ekList in enumerate(ekListsCopy):
        while len(ekList) > 0:
            ...
            inEkRemain.remove(j)
            for point in range(
                ekList[0][1] - sensorRange, ekList[0][1] + sensorRange, 10
            ):
            ...
            if len(inEkRemain) >= (len(ekListsCopy) + 1) // 2:
                ...
                for k in inEkRemain:
                    ekLists[k].append(
                        (ekList[0][0], int(sum(inEkTime) // len(inEkTime) * 10))
                    )
                for k in range(len(inEkIdx)):
                    ekListsCopy[inEkIdx[k]].remove((ekList[0][0], inEkTime[k] * 10))
            ...
            if ek[i][0] == expected:
                ...
                if duplicate != []:
                    ...
                    ek.insert(
                        ...
                        (
                            duplicate[0][0],
                            int(sum([d[1] / 10 for d in duplicate]) // len(duplicate))
                            * 10,
                        ),
                    )
                    duplicate = []
...
) -> tuple[list[list[int]], list[int], list[list[tuple[int, int]]]]:
    ekLists = []
