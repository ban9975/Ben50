...
mode = int(input("0: gesture, 1: length, 2: raw, 3: first round, 4: avg : ")) # 0 for gesture, 1 for length
# mode = 0
trainFile = 'wristband/v7/adi_v3_1ADC_average.xlsx'
testFile = 'wristband/v7/adi_v3_1ADC_average_test.xlsx'
train = importDataRaw.importData(trainFile,mode)
test = importDataRaw.importData(testFile,mode)
if mode == 4:
    train.data.to_excel(r'data_cal/v7/adi_v3_1ADC_average_cal_first_train.xlsx')
    test.data.to_excel(r'data_cal/v7/adi_v3_1ADC_average_cal_first_test.xlsx')
print(trainFile)
